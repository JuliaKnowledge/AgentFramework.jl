---
title: "Group Chat Orchestrations"
author: "Simon Frost"
format:
  html:
    code-fold: false
...
When multiple agents need to collaborate on a problem — debating, reviewing,
or building on each other's ideas — you need a **group chat**. AgentFramework
provides three orchestration patterns with increasing sophistication.

By the end you will know how to:

1. Run a **round-robin** group chat where agents take turns in sequence.
2. Use a **selector-based** group chat with an orchestrator agent or function.
3. Build a **Magentic** orchestration with planning and stall detection.
...
We will use three philosophical personas that debate a topic: an **Optimist**,
a **Pessimist**, and a **Moderator**.
...
For more dynamic conversations, an **orchestrator agent** decides who speaks
next based on the conversation so far. This is useful when you want experts
to be called upon only when relevant.
...
The `GroupChatState` passed to selection and termination functions contains:
`conversation::Vector{Message}`, `participant_ids::Vector{String}`,
`round::Int`, `last_speaker::Union{Nothing, String}`, and
`metadata::Dict{String, Any}`.

## Magentic Orchestration

For complex multi-step tasks, the **Magentic** pattern adds a planning layer.
A manager creates a task ledger, selects participants, tracks progress, and
replans if the conversation stalls.
...
- **`MagenticBuilder`** adds planning, progress tracking, and replanning.
- All patterns produce a standard `Workflow` via `build()` and use the same
  `run_workflow` / event inspection API.
- Use `intermediate_outputs = true` to stream agent responses as they happen.

Next, see [17 — Evaluation](../17_evaluation/17_evaluation.qmd) to learn how
to measure and benchmark your agents.
