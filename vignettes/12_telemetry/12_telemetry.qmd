...
When agents call LLMs, invoke tools, and orchestrate workflows, understanding
what happened (and how long it took) is essential for debugging, performance
tuning, and cost tracking. This vignette covers the built-in telemetry system.
...
6. Inspect collected spans for operation names, durations, and token usage.
...
Without telemetry, debugging "why did the agent give that answer?" requires
manual logging. With spans, you get structured traces that show exactly what
happened, when, and how long each step took.
...
AgentFramework follows [OpenTelemetry Semantic Conventions for GenAI](https://opentelemetry.io/docs/specs/semconv/gen-ai/)
for attribute naming. Key constants live in the `GenAIConventions` module:
...
Emits structured log messages for each span — suitable for production where
you want spans in your log aggregation system:
...
After running the agent, the backend contains spans for each pipeline layer:
...
The exact number of spans depends on whether the agent calls tools. Each tool
invocation adds a `tool.invoke` span.

## Complete Observability Example

Putting it all together — an agent with tools, instrumented for full tracing:
...
The telemetry system is designed to be lightweight and composable. Implement
`AbstractTelemetryBackend` with a custom `record_span!` method to integrate
with OpenTelemetry, Datadog, or any observability platform.
