...
So far every agent has been built imperatively — constructing types, wiring
tools, and calling `run_agent` from Julia code. **Declarative agents** flip this
around: you describe the agent (or an entire workflow) in a YAML or JSON file
and let the framework materialize it at runtime. This makes agent definitions
portable, version-controllable, and editable by non-programmers.
...
3. Register tools and clients so declarative definitions can reference them by name.
...
Imperative agent construction is flexible but couples every detail to Julia
source code. Declarative definitions provide several benefits:
...
The simplest declarative agent is a `Prompt` kind — an agent defined entirely by
its instructions and model options. Pass the YAML string to `agent_from_yaml`
along with a default chat client:
...
The returned `agent` is a regular `Agent` — you interact with it the same way as
any imperatively constructed agent:
...
For production use, store definitions in `.yaml` files and load them with
`agent_from_file`:
...
The file `agents/weather.yaml` contains the same YAML shown above. The function
reads the file, parses it, and delegates to `agent_from_yaml`.

You can also load JSON files — the loader detects the format automatically:
...
When a YAML definition references tools or clients **by name**, those names must
be resolved at load time. Use the global registry functions to bind names to
live objects before loading:
...
Declarative definitions extend beyond single agents to multi-step workflows.
A workflow YAML specifies executors, edges, and the start/output nodes:
...
Executors in the YAML are resolved by `id` — you register handler functions
before loading:
...
Live agents and workflows can be serialized back to YAML or JSON, enabling
inspection, modification, and re-loading:
...
Round-trip fidelity means you can programmatically build an agent, export it,
hand-edit the YAML, and re-import without loss.

## Environment Variable Substitution

Declarative configs often need secrets or deployment-specific values that should
not be hard-coded. Use `${VAR_NAME}` syntax for environment variable references:
...
Here is a complete example that defines a tool, registers it, and loads a
declarative agent that uses it:
...
1. Declarative definitions decouple **what** an agent does from **how** it is instantiated.
2. YAML and JSON formats are interchangeable — pick whichever suits your workflow.
3. The **registry** (`register_client!`, `register_tool!`, `register_handler!`) bridges named references in YAML to live Julia objects.
4. **Round-trip serialization** lets you inspect, modify, and re-import definitions without loss.
5. **Environment variable substitution** keeps secrets out of config files.

Next, see [20 — Multimodal Agents](../20_multimodal/20_multimodal.qmd)
to work with images, audio, and other media types in agent conversations.
